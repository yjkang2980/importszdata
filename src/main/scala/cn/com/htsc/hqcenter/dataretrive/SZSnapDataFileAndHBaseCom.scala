...
import cn.com.htsc.hqcenter.dataretrive.SZSnapDataFileAndHBaseCom.conf
...
/**
  ...
  * @author 010571 -----------20170119tick可能需要重新导入....(在批量导入时替换了jar包刚好冲击了这一天的数据)
  * @version $Id:
  *
  */
object SZSnapDataFileAndHBaseCom {
  val conf= new org.apache.hadoop.conf.Configuration();
  val hdfs = org.apache.hadoop.fs.FileSystem.get(new java.net.URI("hdfs://nameservice1"),conf)
  ...
  var resultDir="/user/u010571/dataretrieve/"+year+month+day+"/snaplevel/statistic"
  var dataOutDir="/user/u010571/dataretrieve/"+year+month+day+"/snaplevel/szHasHbaseNull"
  var dataOutDir2="/user/u010571/dataretrieve/"+year+month+day+"/snaplevel/HbaseHasszNull"
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("hqszsnapdatacompare").set("spark.yarn.submit.file.replication","1")
   ...
   // val sdf: SimpleDateFormat = new SimpleDateFormat("yyyyMMddHHmmssSSS")
    ...
    resultDir="/user/u010571/dataretrieve/"+year+month+day+"/snaplevel/statistic"
    dataOutDir="/user/u010571/dataretrieve/"+year+month+day+"/snaplevel/szHasHbaseNull"
    dataOutDir2="/user/u010571/dataretrieve/"+year+month+day+"/snaplevel/HbaseHasszNull"
    ...
    if(null!=result){
      ...
      /*if(hdfs.exists(new Path(resultDIr+"/part-00000"))){
        ...
        UPloadToFtp.uploadToFtp(in, "/hjp/"+year+month+day+"/snaplevel/statistic", "part-00000.txt")
      }
      if(hdfs.exists(new Path(dataOutDir+"/part-00000"))){
        val in = hdfs.open(new Path(dataOutDir+"/part-00000"))
        UPloadToFtp.uploadToFtp(in, "/hjp/"+year+month+day+"/snaplevel/szHasHbaseNull", "part-00000.txt")
      }*/
    }
  }
  ...
  def insertSnapRec(sub1: RDD[Row], hiveContext: HiveContext, sc: SparkContext, ymd: String) = {
    ...
    val snapsql="select  * from table_app_t_md_snap t, mdc_prdtInfos p,table_app_t_md_snap_level q,mdc_snap_intersectdata r where t.year='"+ymd.substring(0,4)+"' and t.month='"+ymd.substring(4,6)+"' and t.day='"+ymd.substring(6,8)+"' and q.year='"+ymd.substring(0,4)+"' and q.month='"+ymd.substring(4,6)+"' and q.day='"+ymd.substring(6,8)+"' and t.securityid=q.securityid and t.origtime=q.origtime and t.securityid=r.securityid and t.origtime=r.origtime and p.sectype='2' and p.prdtid=t.securityid "
  ...
  def checkSnap(sc:SparkContext, hiveContext: HiveContext, year:String, mon:String, day:String): String ={
   //  val secIDs:DataFrame=hiveContext.sql("select p.securityid,'"+year+"' as year,'"+mon+"' as month,'"+day+"' as day from mdc_center_prdt_info p where  p.sectype='2' and instr(p.securityid,'.SZ')>0")
    ...
    var oneRdd=secIDs.rdd.flatMap {
      case Row(securityid: String, year: String, month: String, day: String) => {
        ...
        val table: org.apache.hadoop.hbase.client.Table = conn.getTable(TableName.valueOf("MDC:MDStockRecord"))
        val scan = new Scan();
       // scan.setMaxVersions
   ...
   // val oner=oneRdd.filter{case(row1,row2)=>{
    ...
    //  return null;
    //}
    println("hbase中查询的"+year+"-"+month+"--"+day+"　ｈｂａｓｅ中数据量："+onerc1+" 过滤之后的数据量："+onerc)

...
/**
  *用spark对逐笔委托数据文件进行校验和缺失导入，缺失导入类四个参数分别表示：年、月、日、是否在校验完成后将缺失数据入库,T表示入库
  * @author 010571 
  * @version $Id:
  *
  */
object SZOrderDataFileAndHBaseCom {
  ...
  var resultDir="/user/u010571/dataretrieve/"+year+month+day+"/order/statistic"
  var dataOutDir="/user/u010571/dataretrieve/"+year+month+day+"/order/szHasHbaseNull"
  var dataOutDir2="/user/u010571/dataretrieve/"+year+month+day+"/order/HbaseHasszNull"

  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("hqszorderdatacompare")
   ...
   // val day="20"
    ...
    resultDir="/user/u010571/dataretrieve/"+year+month+day+"/order/statistic"
    dataOutDir="/user/u010571/dataretrieve/"+year+month+day+"/order/szHasHbaseNull"
    dataOutDir2="/user/u010571/dataretrieve/"+year+month+day+"/order/HbaseHasszNull"
    ...
    val result=checkOrder(sc,hiveContext,year,month,day);
    if(null!=result){
      ...
      /*if(hdfs.exists(new Path(resultDIr+"/part-00000"))){
        ...
        UPloadToFtp.uploadToFtp(in, "/hjp/"+year+month+day+"/order/statistic", "part-00000.txt")
      }
      if(hdfs.exists(new Path(dataOutDir+"/part-00000"))){
        val in = hdfs.open(new Path(dataOutDir+"/part-00000"))
        UPloadToFtp.uploadToFtp(in, "/hjp/"+year+month+day+"/order/szHasHbaseNull", "part-00000.txt")
      }*/
    }

  }

  def insertOrderRec(sub: RDD[Row], hiveContext: HiveContext, sc: SparkContext, ymd: String) = {
    ...
    val indexData=hiveContext.sql("select *from table_app_t_md_order t where  t.year='"+ymd.substring(0,4)+"' and t.month='"+ymd.substring(4,6)+"' and t.day='"+ymd.substring(6,8)+"'")
    // prdDF.cache();
    ...
    //000977,20180330093234590,648070
  ...
  def checkOrder(sc:SparkContext, hiveContext: HiveContext, year:String, mon:String, day:String): String ={
    val secIDs:DataFrame=hiveContext.sql("select distinct(t.securityid),t.year,t.month,t.day from table_app_t_md_order t where  t.year='"+year+"' and t.month='"+mon+"' and t.day='"+day+"'")
    ...
    var oneRdd=secIDs.rdd.flatMap {
      case Row(securityid: String, year: String, month: String, day: String) => {
        ...
        val table: org.apache.hadoop.hbase.client.Table = conn.getTable(TableName.valueOf("MDC:MDOrderRecord"))
        val scan = new Scan();
       // scan.setMaxVersions
        ...
        if(size2<=0){
          ...
          while (it2.hasNext) {
            ...
            if (null != next2) {
              ...
              val applseqnum = new String(next2.getValue(Bytes.toBytes("Detail"), Bytes.toBytes("OrderIndex")))
    ...
    val oner=oneRdd.map({case(row1,row2,row3)=>(row1,row2,row3)}).filter{case (row1,row2,row3)=>{
      // println("row值："+row1)
   ...
   // }).map(row=>Row(row._1,row._2,row._3))
    ...
    val secIDs2:DataFrame=hiveContext.sql("select t.securityid,cast(t.origtime as string) origtime,cast(t.applseqnum as string)applseqnum from table_app_t_md_order t where  t.year='"+year+"' and t.month='"+mon+"' and t.day='"+day+"'")
   ...
   // sub.sortBy()
    ...
    if(insertRec==true){
      insertOrderRec(sub,hiveContext,sc,year+mon+day)
  ...
  def convertOrderType(row: Row)= {
    ...
    val orderBSFlag=row.getAs[String]("side")
    ...
    if(orderBSFlag.equals("1") || orderBSFlag.equals("G")){
      p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("OrderBSFlag"), Bytes.toBytes("1"))
    }
    if(orderBSFlag.equals("2") || orderBSFlag.equals("F")){
      p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("OrderBSFlag"), Bytes.toBytes("2"))
    }

    if(ExpirationType!=null && !ExpirationType.equals("NULL")) {
      p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("ExpirationType"), Bytes.toBytes(ExpirationType.toString))
    }
    // p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("ClosePx"), Bytes.toBytes())
    if(null!=ExpirationDays && !ExpirationDays.equals("NULL")){
      p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("ExpirationDays"), Bytes.toBytes(ExpirationDays.toString))
    }
    if(null!=contactor && !contactor.equals("NULL")){
      p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("Contactor"), Bytes.toBytes(contactor))
    }
    if(null != contactinfo && !contactinfo.equals("NULL")){
      p.addColumn(Bytes.toBytes("Detail"), Bytes.toBytes("ContactInfo"), Bytes.toBytes(contactinfo))
    }

    if(null!=confirmid && !confirmid.equals("NULL")){

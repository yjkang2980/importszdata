...
/**
  ...
  *
  */
object SZBondDataFileAndHBaseCom {
  ...
  var resultDir="/user/u010571/dataretrieve/"+year+month+day+"/bond/statistic"
  var dataOutDir="/user/u010571/dataretrieve/"+year+month+day+"/bond/szHasHbaseNull"
  var dataOutDir2="/user/u010571/dataretrieve/"+year+month+day+"/bond/HbaseHasszNull"
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("hqszbonddatacompare")
   ...
   // val sdf: SimpleDateFormat = new SimpleDateFormat("yyyyMMddHHmmssSSS")
    ...
    resultDir="/user/u010571/dataretrieve/"+year+month+day+"/bond/statistic"
    dataOutDir="/user/u010571/dataretrieve/"+year+month+day+"/bond/szHasHbaseNull"
    dataOutDir2="/user/u010571/dataretrieve/"+year+month+day+"/bond/HbaseHasszNull"
    ...
    if(null!=result){
     ...
     /* if(hdfs.exists(new Path(resultDIr+"/part-00000"))){
        ...
        UPloadToFtp.uploadToFtp(in, "/hjp/"+year+month+day+"/bond/statistic", "part-00000.txt")
      }
      if(hdfs.exists(new Path(dataOutDir+"/part-00000"))){
        val in = hdfs.open(new Path(dataOutDir+"/part-00000"))
        UPloadToFtp.uploadToFtp(in, "/hjp/"+year+month+day+"/bond/szHasHbaseNull", "part-00000.txt")
      }*/
    }
  }
  ...
  def insertSnapRec(sub1: RDD[Row], hiveContext: HiveContext, sc: SparkContext, ymd: String) = {
    ...
    val snapsql="select  * from table_app_t_md_snap t, mdc_prdtInfos p,table_app_t_md_snap_level q,mdc_snap_intersectdata r where t.year='"+ymd.substring(0,4)+"' and t.month='"+ymd.substring(4,6)+"' and t.day='"+ymd.substring(6,8)+"' and q.year='"+ymd.substring(0,4)+"' and q.month='"+ymd.substring(4,6)+"' and q.day='"+ymd.substring(6,8)+"' and t.securityid=q.securityid and t.origtime=q.origtime and t.securityid=r.securityid and t.origtime=r.origtime and p.sectype='4' and p.prdtid=t.securityid "
    ...
    sc.hadoopConfiguration.set(TableOutputFormat.OUTPUT_TABLE, "MDC:MDBondRecord")
    //指定输出格式和输出表名
   ...
   //  val secIDs:DataFrame=hiveContext.sql("select p.securityid,'"+year+"' as year,'"+mon+"' as month,'"+day+"' as day from mdc_center_prdt_info p where  p.sectype='2' and instr(p.securityid,'.SZ')>0")
    ...
    var oneRdd=secIDs.rdd.flatMap {
      case Row(securityid: String, year: String, month: String, day: String) => {
        ...
        val table: org.apache.hadoop.hbase.client.Table = conn.getTable(TableName.valueOf("MDC:MDBondRecord"))
        val scan = new Scan();
       // scan.setMaxVersions

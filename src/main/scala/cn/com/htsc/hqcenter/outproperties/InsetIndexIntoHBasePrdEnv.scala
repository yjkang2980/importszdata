...
/**
  ...
  *
  */
object InsetIndexIntoHBasePrdEnv {
    ...
    for(day <- days){
      if(day >=startDay && day <=endDay){
       ...
       // insertEvertDay(hiveContext,prdDF,year,month,daye,zkQurom)
        ...
        val indexSql="select t.origtime,t.securityid,t.securityidsource,t.mdstreamid, " +
          ...
          "  t.year='"+year+"' and t.month='"+mon+"' and t.day='"+daye+"' and t.origtime between '"+sstart+"' and '"+sendt+"'"
      ...
      //  jobConf.setOutputFormatClass(classOf[TableOutputFormat[ImmutableBytesWritable]])
        ...
        if(insertData.equals("T")){
          ...
          println("第一个记录：time:"+origtime+" sec:"+secid+" rowKey:"+rowKey)
        }
        println("导入"+year+"/"+mon+"/"+daye+"的index指数数据结束")

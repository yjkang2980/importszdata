...
object FileList {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val snaplevelfls: Array[FileStatus] = hdfs.listStatus(new Path("hdfs://nameservice1/user/u010571/snapshotlevelnew"))
    val orderfls: Array[FileStatus] = hdfs.listStatus(new Path("hdfs://nameservice1/user/u010571/ordernew"))
    val tradefls: Array[FileStatus] = hdfs.listStatus(new Path("hdfs://nameservice1/user/u010571/tradenew"))
    val indexfls: Array[FileStatus] = hdfs.listStatus(new Path("hdfs://nameservice1/user/u010571/indexnew"))
    val stockinfofls: Array[FileStatus] = hdfs.listStatus(new Path("hdfs://nameservice1/user/u010571/stockinfonew"))
    val stockstatusfls: Array[FileStatus] = hdfs.listStatus(new Path("hdfs://nameservice1/user/u010571/stockstatusnew"))
    ...
    //遍历添加snapshot的数据
    myFunc(hiveContext,hdfs,snapfls,snapt)

    //遍历添加snap_level的行情
    myFunc(hiveContext,hdfs,snaplevelfls,snaplevelt)

    //遍历order行情
    myFunc(hiveContext,hdfs,orderfls,ordert)


    //遍历trade
    myFunc(hiveContext,hdfs,tradefls,tradet)

    //遍历index
    myFunc(hiveContext,hdfs,indexfls,indext)
  ...
  def checkOrderRec(hiveContext: HiveContext,year:String,mon:String,day:String)={
     ...
     val timeAndIndexIncreaseSql="select count(*) from (" +
       ...
       " from table_app_t_md_order t where t.year='2016' and t.month='06' and t.day='16') AA where AA.applseqnum<=AA.lastseqNum  " +
  ...
  def checktradNumValueSumEqualLastRec(hiveContext: HiveContext,year:String,mon:String,day:String)={
    val tradepricesql="select count(*)from " +
  ...
  /**
    ...
    * @param tablename
    */
  def myFunc(hiveContext: HiveContext,hdfs: FileSystem,p1: Array[FileStatus],tablename:String) = {
  ...
  /**
    ...
    */
  def checkEveryDay(hiveContext: HiveContext,hdfs: FileSystem,p1: Array[FileStatus]) = {
